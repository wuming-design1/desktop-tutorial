...
import type { SummaryData, ReportData, HealthStatus, PieSlice, BarItem } from '../utils/types'

export async function getDashboardSummary(userId: string): Promise<SummaryData> {
  ...
  let activeRepos = 0
  let newIssues = 0
  const updates: SummaryData['updates'] = []
  ...
  if (hasGithub) connectedNames.push('GitHub')
  if (hasFeishu) connectedNames.push('飞书')
...
export async function getReports(userId: string): Promise<ReportData> {
  ...
  const taskDistribution: PieSlice[] = [
  ...
  const memberActivity: BarItem[] = commitTrend.length > 0
