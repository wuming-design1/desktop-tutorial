...
export async function fetchCommitTrend(userId: string): Promise<TrendPoint[]> {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/${parsed.owner}/${parsed.repoName}/commits?since=${since}&per_page=100`,
...
export async function fetchTodayCommits(userId: string): Promise<number> {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/${parsed.owner}/${parsed.repoName}/commits?since=${today}T00:00:00Z&per_page=100`,
      { headers: ghHeaders(token) }
    )
...
export async function fetchRepoInfo(userId: string): Promise<{ openIssues: number; watchers: number }> {
  ...
  const parsed = parseRepo(repo)
  if (!parsed) return { openIssues: 0, watchers: 0 }

  try {
    const res = await fetch(
      `https://api.github.com/repos/${parsed.owner}/${parsed.repoName}`,
      { headers: ghHeaders(token) }
    )
...
/** 获取默认分支名称 */
async function getDefaultBranch(owner: string, repoName: string, token: string): Promise<string> {
...
/** 获取默认分支最新 commit SHA */
async function getLatestSha(owner: string, repoName: string, branch: string, token: string): Promise<string> {
  const res = await fetch(
    `https://api.github.com/repos/${owner}/${repoName}/git/refs/heads/${branch}`,
    { headers: ghHeaders(token) }
  )
  if (!res.ok) throw new Error(`无法获取分支 ${branch} 的信息`)
  const data = await res.json()
  return data.object.sha
}

/** 创建 Git Blob */
async function createBlob(owner: string, repoName: string, content: string, token: string): Promise<string> {
  const res = await fetch(`https://api.github.com/repos/${owner}/${repoName}/git/blobs`, {
    method: 'POST',
    headers: ghHeaders(token),
    body: JSON.stringify({ content, encoding: 'utf-8' }),
  })
  if (!res.ok) throw new Error('创建 Blob 失败')
  const data = await res.json()
  return data.sha
}
...
async function createTree(
  owner: string, repoName: string, baseSha: string, files: FileToUpload[], token: string
): Promise<string> {
  ...
  if (!res.ok) throw new Error('创建 Tree 失败')
  const data = await res.json()
  return data.sha
}
...
async function createCommit(
  owner: string, repoName: string, treeSha: string, parentSha: string, message: string, token: string
): Promise<string> {
...
/** 创建/更新分支引用 */
async function updateBranchRef(owner: string, repoName: string, branch: string, sha: string, token: string): Promise<void> {
...
async function createPR(
  owner: string, repoName: string, title: string, body: string, head: string, base: string, token: string
...
): Promise<{ number: number; html_url: string; branch: string }> {
  const creds = getCredentialsForUser(userId, 'github')
  const token = creds['GITHUB_TOKEN']
  const repo = creds['GITHUB_REPO']

  if (!token || !repo) throw new Error('未配置 GitHub 凭证')
  const parsed = parseRepo(repo)
  if (!parsed) throw new Error('仓库格式无效，应为 owner/repo')
...
export async function createPRFromStaging(
  userId: string,
  options: {
    title: string
    body: string
...
): Promise<{ number: number; html_url: string; branch: string; files: string[] }> {
  const creds = getCredentialsForUser(userId, 'github')
  const token = creds['GITHUB_TOKEN']
  const repo = creds['GITHUB_REPO']
  ...
  if (!token || !repo) throw new Error('未配置 GitHub 凭证')
  const parsed = parseRepo(repo)
  if (!parsed) throw new Error('仓库格式无效')
