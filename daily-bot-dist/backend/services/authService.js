...
export function refreshTokenInternal(userId, email, role) {
    const token = signToken({ userId, email, role });
    return { token };
}
export function updateSettings(email, settings) {
...
export function deleteUser(email) {
    ...
    if (row.role === 'admin') {
        ...
        if (adminCount <= 1)
            throw new AppError(400, '不能删除唯一的管理员账号');
    }
...
export function changeRole(email, role) {
    ...
    if (role !== 'admin' && row.role === 'admin') {
        const adminCount = countAdmins();
        if (adminCount <= 1)
            throw new AppError(400, '不能移除唯一的管理员权限');
    }
    updateUserRole(email, role);
    return rowToUser({ ...row, role: role });
}
